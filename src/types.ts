...
export type SessionEvent = SessionUpdateEvent | MessageEvent | PermissionRequestEvent
...
export function isSessionEvent(event: unknown): event is SessionEvent {
  return (
    ...
    event !== null &&
    'type' in event &&
    ['session_update', 'message', 'permission_request'].includes(event.type as string)
  )
}