    ...
    fc.assert(
      fc.property(
        ...
        (sessions, servers, groupBy) => {
          ...
          const groupedSessionIds = new Set(allGroupedSessions.map(s => s.id))
          ...
          for (let i = 0; i < sorted.length - 1; i++) {
            ...
            switch (sortBy) {
              case 'name':
                ...
                break
              case 'activity':
                expect(current.lastActivity).toBeGreaterThanOrEqual(next.lastActivity)
                break
              case 'created':
    ...
    fc.assert(
      fc.property(
        ...
        (sessions, servers, groupBy, sortBy) => {
          ...
          const originalSessionCount = groups.reduce((sum, g) => sum + g.sessions.length, 0)
          const sortedSessionCount = sortedGroups.reduce((sum, g) => sum + g.sessions.length, 0)
        ...
        (sessions, showOnlyActive, searchTerm) => {
          ...
          if (searchTerm) {
            ...
            for (const session of filtered) {
              ...
              const matchesProject = session.project?.toLowerCase().includes(term) || false
              const matchesBranch = session.branch?.toLowerCase().includes(term) || false
        ...
        (sessions) => {
          ...
          function countSessionsInHierarchy(sessions: Session[]): number {
            return sessions.reduce((count, session) => {
              return count + 1 + (session.children ? countSessionsInHierarchy(session.children) : 0)
            }, 0)
          }
    ...
    fc.assert(
      fc.property(
        ...
        (sessions) => {
          ...
          const expectedCost = sessions.reduce((sum, s) => sum + (s.cost || 0), 0)
          const expectedTokens = sessions.reduce((sum, s) => sum + (s.tokens || 0), 0)
