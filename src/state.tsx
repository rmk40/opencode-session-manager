...
import { AppState, Server, Session, ViewMode, GroupMode, SortMode, NotificationState, AppError } from './types'
...
function stateReducer(state: AppState, action: StateAction): AppState {
  switch (action.type) {
    ...
    case 'REMOVE_SESSION':
      ...
      const selectedSessionId = state.selectedSessionId === action.sessionId 
        ? undefined 
...
export function useSelectedSession(): Session | undefined {
  const { state } = useAppState()
  return state.selectedSessionId ? state.sessions.get(state.selectedSessionId) : undefined
}
