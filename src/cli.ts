...
export function parseCliArgs(args: string[] = process.argv.slice(2)): CLIOptions {
  try {
    ...
    const mode = positionals[0] as CLIOptions['mode'] || values.mode as CLIOptions['mode']
    ...
    return {
      ...
      mockServers: values['mock-servers'] ? parseInt(values['mock-servers'], 10) : undefined,
      testScenarios: values['test-scenarios'] ? values['test-scenarios'].split(',') : undefined,
      help: values.help,
      version: values.version
    }
  } catch (error) {
    console.error('Error parsing command line arguments:', error instanceof Error ? error.message : error)
    process.exit(1)
  }
}
...
export async function runTestMode(options: CLIOptions): Promise<void> {
  ...
  if (options.testScenarios) {
    ...
    if (scenariosToRun.length === 0) {
      ...
      console.log('Available scenarios:', builtInScenarios.map(s => s.name).join(', '))
      process.exit(1)
    }
  }
  ...
  const results = await headlessRunner.runScenarios(scenariosToRun)
...
export async function runMockMode(options: CLIOptions): Promise<void> {
  ...
  for (let i = 0; i < serverCount; i++) {
    ...
    const server = await mockServerManager.createServer(serverId, serverName, port)
    servers.push(server)
...
export async function runStatusMode(options: CLIOptions): Promise<void> {
  ...
  for (const [id, server] of servers) {
    console.log(`  ${server.name} (${server.url}) - ${server.isHealthy ? 'Healthy' : 'Unhealthy'}`)
  }
...
async function daemonize(options: CLIOptions): Promise<void> {
