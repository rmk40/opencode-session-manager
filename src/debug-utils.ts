...
export class PacketInspector {
  ...
  inspectUDPPacket(packet: UDPPacket, source: string): void {
    ...
    if (this.logger) {
      this.logger.debug(
        `UDP packet received from ${source}`,
        inspection.packet,
      );
    }
  }
   ...
   */
  private validateUDPPacket(packet: UDPPacket): {
    valid: boolean;
    issues: string[];
  } {
    const issues: string[] = [];
    ...
    if (packet.type === "announce") {
      ...
      if (
        !announcePacket.serverUrl ||
        typeof announcePacket.serverUrl !== "string"
      ) {
        issues.push("Invalid or missing serverUrl for announce packet");
      }
      if (
        !announcePacket.serverName ||
        typeof announcePacket.serverName !== "string"
      ) {
   ...
   */
  private validateSSEEvent(event: SSEEvent): {
    valid: boolean;
    issues: string[];
  } {
    const issues: string[] = [];
...
export class PerformanceMonitor {
  ...
  endOperation(operationId: string, operationType: string): number {
    ...
    if (!startTime) {
      if (this.logger) {
        this.logger.warn(
          `No start time found for operation: ${operationType} (${operationId})`,
        );
      }
      return 0;
    }
    ...
    if (this.logger) {
      this.logger.trace(
        `Completed operation: ${operationType} (${operationId}) in ${duration.toFixed(2)}ms`,
      );
    }
