...
import { Session, SessionStatus, Message, MessageType, AnnouncePacket, ShutdownPacket } from './types'
...
export class MockSessionGenerator {
  ...
  generateSession(serverId: string, serverUrl: string, overrides?: Partial<Session>): Session {
    ...
    return {
      ...
      parentId: undefined,
      childIds: [],
      project: Math.random() > 0.5 ? `project-${Math.floor(Math.random() * 5) + 1}` : undefined,
      branch: Math.random() > 0.5 ? `feature-branch-${Math.floor(Math.random() * 10) + 1}` : 'main',
      cost: Math.random() * 10,
  ...
  generateMessage(sessionId: string, overrides?: Partial<Message>): Message {
    ...
    const types: MessageType[] = ['user_input', 'assistant_response', 'tool_execution', 'system_message']
    const now = Date.now()
...
export class MockOpenCodeServer extends EventEmitter {
   ...
   */
  addMessage(sessionId: string, message?: Partial<Message>): Message | undefined {
    const session = this.sessions.get(sessionId)
    if (!session) {
      return undefined
    }
  ...
  requestPermission(sessionId: string): Message | undefined {
    ...
    const permissionMessage = this.generator.generatePermissionRequest(sessionId)
    session.messages.push(permissionMessage)
    session.status = 'waiting_for_permission'
    session.lastActivity = Date.now()
    
    this.sessions.set(sessionId, session)
    this.emit('permission_requested', { sessionId, message: permissionMessage })
  ...
  private handleHTTPRequest(req: any, res: any): void {
    ...
    try {
      ...
      } else if (url.pathname.startsWith('/sessions/') && req.method === 'GET') {
        const sessionId = url.pathname.split('/')[2]
        this.handleSessionDetailsRequest(sessionId, res)
      } else if (url.pathname.startsWith('/sessions/') && url.pathname.endsWith('/messages') && req.method === 'POST') {
        const sessionId = url.pathname.split('/')[2]
        this.handleSendMessageRequest(sessionId, req, res)
      } else if (url.pathname.startsWith('/sessions/') && url.pathname.endsWith('/abort') && req.method === 'POST') {
        const sessionId = url.pathname.split('/')[2]
   ...
   */
  private handleSendMessageRequest(sessionId: string, req: any, res: any): void {
    let body = ''
  ...
  private handleSSERequest(req: any, res: any): void {
    ...
    res.write(`data: ${JSON.stringify({ type: 'connected', server_id: this.serverId })}\n\n`)
    ...
    const onMessageAdded = ({ sessionId, message }: { sessionId: string, message: Message }) => {
      const event = {
        type: 'message',
    ...
    const onPermissionRequested = ({ sessionId, message }: { sessionId: string, message: Message }) => {
      const event = {
        type: 'permission_request',
        session_id: sessionId,
  ...
  private startAnnouncements(): void {
    ...
    this.announceTimer = setInterval(announce, 5000) // Every 5 seconds
  }
...
export class MockServerManager {
   ...
   */
  async createServer(serverId?: string, serverName?: string, port?: number): Promise<MockOpenCodeServer> {
