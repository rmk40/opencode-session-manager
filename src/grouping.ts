  ...
  switch (sortBy) {
    ...
    case "activity":
      return sorted.sort((a, b) => {
        const latestA =
          a.sessions.length > 0
            ? Math.max(...a.sessions.map((s) => s.lastActivity))
            : 0;
        const latestB =
          b.sessions.length > 0
            ? Math.max(...b.sessions.map((s) => s.lastActivity))
            : 0;
    ...
    case "created":
      return sorted.sort((a, b) => {
        const latestA =
          a.sessions.length > 0
            ? Math.max(...a.sessions.map((s) => s.createdAt))
            : 0;
        const latestB =
          b.sessions.length > 0
            ? Math.max(...b.sessions.map((s) => s.createdAt))
            : 0;
        return latestB - latestA;
      });
...
export function calculateGroupStatistics(group: SessionGroup): {
  ...
  const errorCount = sessions.filter((s) =>
    ["error", "aborted"].includes(s.status),
  ).length;
...
export function buildSessionHierarchy(sessions: Session[]): Session[] {
  ...
  return rootSessions.map((session) =>
    sortSessionChildren(session, sessionMap, processedIds),
  );
}

function sortSessionChildren(
  session: Session,
  ...
  const children = (session.childIds || [])
    ...
    .filter(
      (child): child is Session =>
        child !== undefined && !processedIds.has(child.id),
    )
    .map((child) => {
...
export function findSessionsByProject(
  sessions: Session[],
  project: string,
  branch?: string,
): Session[] {
...
export function findSessionsByServer(
  sessions: Session[],
  serverId: string,
): Session[] {
