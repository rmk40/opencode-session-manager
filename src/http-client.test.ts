...
const sdkSessionStatusArb = fc.constantFrom(
  "idle",
  "busy",
  "running",
  "pending",
...
const sessionInfoArb = fc.record({
  ...
  child_ids: fc.array(fc.string({ minLength: 1, maxLength: 50 }), {
    maxLength: 10,
  }),
...
const messageArb = fc.record({
  ...
  metadata: fc.option(
    fc.record({
      ...
      cost: fc.option(
        fc.float({ min: 0, max: 100 }).filter((n) => Number.isFinite(n)),
      ),
...
describe("HTTP Client and Session Management", () => {
  it("Property 8: HTTP client pool manages clients correctly", async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        async (serverUrls) => {
          ...
          await Promise.all(serverUrls.map((url) => pool.getClient(url)));
          ...
          for (const url of serverUrls) {
            const client1 = await pool.getClient(url);
            const client2 = await pool.getClient(url);
    ...
    fc.assert(
      fc.property(
        ...
        (sessionInfo, serverId, serverUrl) => {
          const converted = convertSessionInfo(
            sessionInfo.id,
            sessionInfo,
            serverId,
            serverUrl,
          ...
          expect(converted.name).toBe(sessionInfo.title);

          // Status mapping
          if (sessionInfo.status === "pending") {
            expect(converted.status).toBe("waiting_for_permission");
          } else if (sessionInfo.status === "running") {
            expect(converted.status).toBe("busy");
          } else {
            expect(converted.status).toBe(sessionInfo.status);
          }

          expect(converted.isLongRunning).toBe(sessionInfo.is_long_running);
          expect(converted.parentId).toBe(sessionInfo.parentID || undefined);
          expect(converted.childIds).toEqual(sessionInfo.child_ids);
          expect(converted.project).toBe(sessionInfo.project || undefined);
          expect(converted.branch).toBe(sessionInfo.branch || undefined);
          expect(converted.cost).toBe(sessionInfo.cost || undefined);
          expect(converted.tokens).toBe(sessionInfo.tokens || undefined);
    ...
    fc.assert(
      fc.property(
        ...
        (sessionDetails, serverId, serverUrl) => {
          const converted = convertSessionDetails(
            sessionDetails,
            serverId,
            serverUrl,
          );
          ...
          expect(converted.serverId).toBe(serverId);
          expect(converted.serverUrl).toBe(serverUrl);
          expect(converted.name).toBe(sessionDetails.title);

          // Status mapping
          if (sessionDetails.status === "pending") {
            expect(converted.status).toBe("waiting_for_permission");
          ...
          expect(converted.messages).toHaveLength(
